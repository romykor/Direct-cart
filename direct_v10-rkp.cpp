...
// Direct problem
//
// 30-07-2018  -  v. 10  - quad-precision
// Runge-Kutta 4  -  parallel
...
void RKP( quadfloat &s, quadfloat s0, quadfloat V0[], quadfloat **res);
...
int main()
{
	...
	quadfloat f,e2;
    quadfloat dab[6]; 
	quadfloat *x0,*y0,*z0,*xe,*ye,*ze,*a0,*ae,*si,*cp,*cq,*dKar,C0,*dis;
	quadfloat *dx,*dy,*dz,*da,*dr,delc,maxdC,maxCC,gmaxdC,gmaxCC;
    quadfloat *big_unified,**Res;                     // big matrix Res is only useful for detailed statistics
	quadfloat *x,*y,*z,*az,*s,C,dC,CC;
...
#pragma omp parallel for shared(Res, big_unified)         // big matrix Res is only useful for detailed statistics
	...
	nam_res  = "direct_resq-"  + fil1 + "RKp" + fil2 + ".txt";
	nam_comp = "direct_compq-" + fil1 + "RKp" + fil2 + ".txt";
	nam_stat = "direct_statq-" + fil1 + "RKp" + fil2 + ".txt";
...
//#pragma omp parallel for  // ???
	...
	for (i = 0 ; i < ng ; i++)  {		
		...
		direct(Res,x0[i],y0[i],z0[i],a0[i],si[i],x[i],y[i],z[i],az[i],s[i],C0,C,dC,maxdC,CC,maxCC);         // big matrix Res is only useful for detailed statistics
...
void direct( quadfloat **res, quadfloat x0, quadfloat y0, quadfloat z0, quadfloat a0, quadfloat s0, quadfloat &xf, quadfloat &yf, quadfloat &zf, quadfloat &azf, quadfloat &sf, quadfloat &C0, quadfloat &Cf, quadfloat &dCf, quadfloat &maxdC,  quadfloat &CCf, quadfloat &maxCC)
{
	...
	RKP(s,s0,v0,res);          // big matrix res is only useful for detailed statistics
