...
#pragma omp parallel for shared(Res, big_unified)
	...
	nam_res="direct_resq-"+fil1+"opt"+fil2+".txt";
	nam_comp="direct_compq-"+fil1+"opt"+fil2+".txt";
	nam_stat="direct_statq-"+fil1+"opt"+fil2+".txt";
...
void direct( quadfloat **res, quadfloat x0, quadfloat y0, quadfloat z0, quadfloat a0, quadfloat s0, quadfloat &xf, quadfloat &yf, quadfloat &zf, quadfloat &azf, quadfloat &sf, quadfloat &C0, quadfloat &Cf, quadfloat &dCf, quadfloat &maxdC,  quadfloat &CCf, quadfloat &maxCC)
{
	...
	quadfloat x0p,y0p,z0p,h0,r0,tol=1.0e-09;  //  instead of -07
...
// ----------------  this section applies Runge-Kutta 4th order
	 ...
	 // summation and residuals
		for (j=0;j<6;j++)  {
			w[j]=w[j]+h*(k1[j]+2.*k2[j]+2.*k3[j]+k4[j])/6.;
			res[i+1][j]=w[j];
		}
//  -------------------------  end of RK-4
//*/
